...
class CNN(nn.Module):
    ...
    def __init__(self, number_actions):
        ...
        self.convolution1 = nn.Conv2d(in_channels = 1, out_channels = 32, kernel_size = 3) #32 filters and kernel size of 3X3
        self.convolution2 = nn.Conv2d(in_channels = 32, out_channels = 32, kernel_size = 2) #32 filters and kernel size of 2X2
    ...
    def forward(self, x, hidden=None):
        x = F.sigmoid(F.avg_pool2d(self.convolution1(x), 3, 2)) #Applying maxpool and then sigmoid ---------------------------------
        x = F.tanh(F.avg_pool2d(self.convolution2(x), 3, 2)) #Applying maxpool and then sigmoid
        x = self.dropout1(x)
        x = F.sigmoid(F.avg_pool2d(self.convolution3(x), 3, 2)) #Applying maxpool and then sigmoid
    ...
    def count_neurons(self, image_dim):
        ...
        x = F.sigmoid(F.avg_pool2d(self.convolution1(x), 3, 2)) #Applying maxpool of kernel size 3X3 and stride of 2 -------
        x = F.sigmoid(F.avg_pool2d(self.convolution2(x), 3, 2)) #Applying maxpool of kernel size 3X3 and stride of 2
        x = F.sigmoid(F.avg_pool2d(self.convolution3(x), 3, 2)) #Applying maxpool of kernel size 3X3 and stride of 2
